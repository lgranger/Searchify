...
func getAlbumData(searchTerm) {
    
    Alamofire.request(.GET, "https://api.spotify.com/v1/search?query=\(searchTerm.text)&limit=10&type=album")
        .responseJSON { response in

            let albumData = JSON(response.result.value![0]) //need to figure out exactly how this come isn
